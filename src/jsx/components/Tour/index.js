...
import { getTours } from '../../../handeApisMethods/tours';
import { deleteTour } from '../../../handeApisMethods/tours';
import { ToastContainer, toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
import { Link } from "react-router-dom";
// import {format} from 'date-fns';
// import { getTour } from '../../../handeApisMethods/tour';
...
const Tours = () => {
	...
	const [tourFullTitle, setTourFullTitle] = useState('');
	const [tourId, setTourId] = useState(''); // the one on edit
	...
	const columns =
		[
			...
			},
			{
				...
				Cell: ({ value }) => {
					return <img src={url + value} alt="Tour Thumbnail" style={{width: 50, height: 50, objectFit: "cover", borderRadius: 15, border: "1px solid white"}} />;
				},
			},
			{
				Header: 'Title',
			...
			},
			{
				...
				Cell: ({ row }) => (
					<>
						<Link className='btn-sm btn m-2 btn-success' to={"/tour/edit/" + row.original.id}><i class="fas fa-pencil-alt"></i></Link>
	...
	const handleShowDeleteWarning = (tour) => {
		setTourId(tour.id)
		setTourKey(tour.key)
		setTourFullTitle(tour.titles[0]["title"])
	...
	useEffect(() => {
		getTours().then(async res => {
			await setTours(res.data)
				setShowTable(true)
		})
					...
					<div className="row">
						{showTable && (
							<TableComponent
								...
								notDataFoundMsg="There is no Added Tour yet!" />
						)}
						<Modal
							className="fade"
						...
						>
							<div role="alert" class="fade notification alert alert-danger show m-0" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', width: '100%' }}>
								<h3 className='text-danger text-center'>Are you sure you want to delete <br /> {tourFullTitle} tour</h3>
								<p>Note this would delete any price wrote in this tour</p>
				...
				<div className="card-body">
					<div className="table-responsive">
						<div className='d-flex justify-content-between align-items-center mb-3'>
							...
							<Link className='btn btn-primary' to={"/create-tour"} >
								{props.add_btn}
							</Link>
						</div>
