...
import { updateTour, getTour } from '../../../handeApisMethods/tours';
      ...
      useEffect(() => {
        if (languages && languages.length)
            getTour(id).then(async res => {
                ...
                res.data.intros.map(intro => {
                    let langKey = languages.find(language => language.id == intro.language_id).key
                ...
                res.data.locations.map(location => {
                    let langKey = languages.find(language => language.id == location.language_id).key

                    if (langKey)
                    setLocations((prevState) => ({
                        ...prevState,
                        [langKey]: location.location,
                    }));
                })
                res.data.transportations.map(transportation => {
                    let langKey = languages.find(language => language.id == transportation.language_id).key

                    if (langKey)
                    setTransportations((prevState) => ({
                        ...prevState,
                        [langKey]: transportation.transportation,
                    }));
                })
                setExpired_date(res.data.expired_date)
                setDuration(res.data.days.length)
                ...
                res.data.excludes.map(exclude => {
                    let langKey = languages.find(language => language.id == exclude.language_id).key

                    if (langKey)
                    setExcludes((prevState) => ({
                        ...prevState,
                        [langKey]: exclude.exclude,
                    }));
                })
                ...
                setNumOfDays(repeatArray2)
                res.data.days.map((day, index) => {
                    day.titles.map(title => {
                        ...
                        if (langKey)
                            setDays((prevState) => ({
                                ...prevState, 
                                [index + 1]: {
                                    ...(prevState[index + 1] ? prevState[index + 1] : null),
                                    titles: {
                                        ...(prevState[index + 1] &&  prevState[index + 1]["titles"] ? prevState[index + 1]["titles"]  : null),
                                        [langKey]: title.title
                                    }
                                }
                            }));            
                        ...
                        if (langKey)
                            setDays((prevState) => ({
                                ...
                                [index + 1]: {
                                    ...(prevState[index + 1] ? prevState[index + 1] : null),
                                    descriptions: {
                                        ...(prevState[index + 1] &&  prevState[index + 1]["descriptions"] ? prevState[index + 1]["descriptions"]  : null),
                    ...
                    setDays((prevState) => ({
                        ...
                        [index + 1]: {
                            ...(prevState[index + 1] ? prevState[index + 1] : null),
                            thumbnail_url: day.thumbnail
                        }
                    }));            
                })
                res.data.packages.map((_package, index) => {
                    _package.titles.map(title => {
                        let langKey = languages.find(language => language.id == title.language_id).key
                    ...
                    _package.descriptions.map(description => {
                        let langKey = languages.find(language => language.id == description.language_id).key
                    ...
                    _package.prices.map(price => {
                        let curr_id = currencies.find(currency => currency.id == price.currency_id).id
                        ...
                        if (curr_id)
                            setPackages((prevState) => ({
                                ...
                                [index + 1]: {
                                    ...(prevState[index + 1] ? prevState[index + 1] : null),
                                    prices: {
                                        ...(prevState[index + 1] &&  prevState[index + 1]["prices"] ? prevState[index + 1]["prices"]  : null),
                                        [curr_id]: price.price
                                    }
                                }
                            }));            
                                            ...
                                            <div className='d-flex gap-3'>
                                                <div className='w-25'> 
                                                    <label htmlFor={"dayThumbnail_" + (index + 1)}>
                                                        {
                                                            ((!days[index + 1] || !days[index + 1]["thumbnail"]) && (!days[index + 1] || !days[index + 1]["thumbnail_url"])) && (
                                                        ...
                                                        {
                                                            ((days[index + 1] && days[index + 1]["thumbnail"]) || (days[index + 1] && days[index + 1]["thumbnail_url"])) && (
                                                                <img src={days[index + 1]["thumbnail"] ? URL.createObjectURL(days[index + 1]["thumbnail"])  : (url + "/" + days[index + 1]["thumbnail_url"])} style={{width: '200px', height: '210px', objectFit: 'cover', padding: '10px', border: '1px solid', borderRadius: '1rem'}} alt={"Day Thumbnail " + (index + 1)} />
                                                            )
                                                        }
                                                    </label> 
                                                    <input 
