...
const Hotels = () => {
			...
			},
			{
				...
				Cell: ({ row }) => (
					<>
						...
						<button className='btn-sm btn m-2 btn-success' onClick={() => handleShowEdit(row.original)}><i class="fas fa-pencil-alt"></i></button>
	...
	const handleShowEdit = (hotel) => {
		...
		setHotelFullName(hotel.name)
		setIsEdit(true)
		setLargeModal(true)
	}
	const handleShowHotel = (hotel) => {
		window.location.href = '/Admin/hotel/' + hotel.id
	}
