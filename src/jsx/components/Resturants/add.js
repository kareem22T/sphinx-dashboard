...
const AddResturant = () => {
  ...
  const handleAddResturant = () => {
    createResturant(thumbnail, names, descriptions, address, addressName, lng, lat).then(res => {
      ...
      </ToastContainer>
      {
        showPageInput && (
          <>
            ...
            <div  className="d-flex gap-3 mt-3">
              <div className='w-25 card p-3 d-flex justify-content-center align-items-center m-0'> 
                  <label htmlFor={"resturant"}>
                      ...
                      {
                          (thumbnail) && (
                              <img src={URL.createObjectURL(thumbnail)} style={{width: '200px', height: '210px', objectFit: 'cover', padding: '10px', border: '1px solid', borderRadius: '1rem'}} alt={"thumbnail"} />
                          )
                      }
                  </label> 
                  <input 
            ...
            <Button className="btn btn-primary w-25" style={{margin: "auto"}} onClick={() => handleAddResturant()}>Create</Button>
          </>
        )
      }
    </>
