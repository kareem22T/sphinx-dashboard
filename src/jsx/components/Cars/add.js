...
import { createCar } from '../../../handeApisMethods/cars';
...
const AddCar = () => {
    ...
    const handleAddCar = () => {
        setShowLoader(true)
        createCar(names, categories, descriptions, address, addressName, lng, lat, prices, carFeatures, selectedFiles, phone).then(res => {
            setShowLoader(false)
            ...
            </ToastContainer>
            {
                showPageInput && (
                    <>
                        ...
                        <Button className="btn btn-primary w-25" style={{ margin: "auto" }} onClick={() => handleAddCar()}>Create</Button>
                    </>
                )
            }
            {
